...
void SimpleOutput::Update()
{
	...
	obs_data_set_int(h264Settings, "buffer_size",
...
AdvancedOutput::AdvancedOutput(OBSBasic *main_) : BasicOutputHandler(main_)
{
	...
	useStreamEncoder = config_get_bool(main->Config(), "AdvOut",
			"RecCopyStream");
	...
	h264Streaming = obs_video_encoder_create("obs_x264", "simple_h264",
			nullptr);
	if (!h264Streaming)
...
{
	int bitrate = config_get_int(main->Config(), "AdvOut",
			"VBitrate");
	bool useBufsize = config_get_bool(main->Config(), "AdvOut",
			"UseBufsize");
	int bufsize = config_get_int(main->Config(), "AdvOut",
			"Bufsize");
	...
	obs_data_set_string(settings, "x264opts", custom);
	obs_data_set_int(settings, "bitrate", bitrate);
	obs_data_set_int(settings, "bufsize", useBufsize ? bufsize : bitrate);
	obs_data_set_bool(settings, "cbr", useCBR);
...
bool AdvancedOutput::StartRecording()
{
	...
	if (!ffmpegRecording && !useStreamEncoder) {
		const char *path = config_get_string(main->Config(),
				"AdvOut", "RecFilePath");
